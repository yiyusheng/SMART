...
# I extract smart from 4000 disks to be the negative set.
...
# 
# # S1.4 Merge and Save
# smartM <- rbind(smartN,smart)
# names(smartM)[5:19] <- names(smartNeed)[c(7:9,21,10:20)]
# smartM <- factorX(smartM)
# save(smartM,file = file.path(dir_data,'smartMerge_smart.Rda'))
...
# S3. Adjust time interval
smartFrame <- expand.grid(seq.POSIXt(as.POSIXct('2014-06-01',tz = 'UTC'),as.POSIXct('2014-11-01',tz='UTC'),by = 'hour'),
                          levels(smartM$sn))
timeseq <- seq.POSIXt(as.POSIXct('2014-06-01',tz = 'UTC'),as.POSIXct('2014-11-01',tz='UTC'),by = 3600*3)
sn <- levels(smartM$sn)
...
adjTime <- function(data){
  ...
  s <- cbind(seqTime = timeseq[idxA],data[idx[idxA],])
  t2 <- proc.time()
...
r <- by(smartM,factor(smartM$sn),adjTime)
save(r,file = file.path(dir_data,'SMARTMerge_IntListA.Rda'))
smartInt <- do.call(rbind,r)
save(smartInt,file = file.path(dir_data,'SMARTMerge_IntA.Rda'))
