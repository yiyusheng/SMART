...
gen_result_thre <- function(df,rate.pos,thre.predict = NULL,thre.lt = NULL){
  
  # generate thre.predict by roc of smart prediction
  if(is.null(thre.predict)){
    thre.predict <- gen_threSet_onresult(df,100000)
  }
...
gen_result <- function(df,rate.pos,thre.predict = NULL,thre = NULL){
